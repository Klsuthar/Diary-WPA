document.addEventListener('DOMContentLoaded', () => {
    ...
    const inspiringQuotes = [
        "Log the present. Unlock the future.",
        "Your data is the future's blueprint.",
        "Capture today. Create tomorrow's AI.",
        "More than data. A digital soul.",
        "Be the source code for humanity.",
        "The most valuable story ever logged.",
        "Every entry forges the ultimate AI.",
        "Powering the next age of intelligence.",
        "Don't just collect. Create a legacy.",
        "Forge the data. Define the future."
    ];
    
    function getRandomQuote() {
        const lastQuoteIndex = localStorage.getItem('lastSplashQuoteIndex');
        let randomIndex;
        
        do {
            randomIndex = Math.floor(Math.random() * inspiringQuotes.length);
        } while (randomIndex == lastQuoteIndex && inspiringQuotes.length > 1);
        
        localStorage.setItem('lastSplashQuoteIndex', randomIndex);
    ...
    function loadAppRecommendations() {
        ...
        const dates = Object.keys(allSavedData).sort((a, b) => new Date(b + 'T00:00:00') - new Date(a + 'T00:00:00')).slice(0, 3);
        ...
        if (datalistElement) {
            datalistElement.innerHTML = '';
            Array.from(appNames).forEach(appName => {
                const option = document.createElement('option');
